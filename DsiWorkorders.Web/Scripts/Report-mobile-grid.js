$(function () {
    ...
    mobileGrid.createDataRow = function (reportrecipient) {

        mobileGrid.createOpenWorkorderRow(reportrecipient)
        //add detail url and priority class to <a> tag
        mobileGrid.$gridContentElement.find('.wo-jquery-mobile-grid-content-row:last').attr('href', mobileGrid.itemDetailUrl + '/' + reportrecipient.Id);

    };

    //method will use open workorder template and will create new data row to append to grid
    mobileGrid.createOpenWorkorderRow = function (reportrecipient) {

        //open workorder template object
        var $template = $($('#wo-mobile-grid-open-template').html());

        //set params
        $template.find('.wo-jquery-mobile-grid-area').text(reportrecipient.AreaName).end()
                .find('.wo-jquery-mobile-grid-email').text(reportrecipient.Emails).end()
