$(function () {
    ...
    mobileGrid.loadData = function (emptyGrid) {
        ...
        $.ajax({
            ...
            data: { //pass params page, pageSize, filter, sort, workOrderType and workOrderDueType to request
                ...
                workOrderType: mobileGrid.workOrderType, workOrderDueType: mobileGrid.workOrderDueType
            },
    ...
    mobileGrid.createDataRow = function (workorder) {
        ...
        if (workorder.Closed == null && workorder.Overdue == false) { //open workorder

            mobileGrid.createOpenWorkorderRow(workorder, priorityClass);

        } else if (workorder.Closed == null && workorder.Overdue) { //open + overdue workorder
            mobileGrid.createOpenOverdueWorkorderRow(workorder, priorityClass);

        } else if (workorder.Closed != null && workorder.Overdue == false) { //closed workorder
            mobileGrid.createClosedWorkorderRow(workorder, priorityClass);

        } else if (workorder.Closed != null && workorder.Overdue) { //closed + overdue workorder
            mobileGrid.createClosedOverdueWorkorderRow(workorder, priorityClass);
        }

        //add detail url and priority class to <a> tag
        mobileGrid.$gridContentElement.find('.wo-jquery-mobile-grid-content-row:last').attr('href', mobileGrid.itemDetailUrl + '/' + workorder.Id)
                                      .parent().addClass(priorityClass);
    };

    //method will use open workorder template and will create new data row to append to grid
    mobileGrid.createOpenWorkorderRow = function (workorder) {

        //open workorder template object
        var $template = $($('#wo-mobile-grid-open-template').html());

        //set params
        $template.find('.wo-jquery-mobile-grid-area').text(workorder.DepartmentAreaName).end()
                .find('.wo-jquery-mobile-grid-department').text(workorder.DepartmentName).end()
                .find('.wo-jquery-mobile-grid-reported-date').text(mobileGrid.formatDate(workorder.Reported)).end()
                .find('.wo-jquery-mobile-grid-due-date').text(mobileGrid.formatDate(workorder.Due)).end()
                .find('.wo-jquery-mobile-grid-days-open').text(workorder.DaysOpen).end()
                .find('.wo-jquery-mobile-grid-details').text(workorder.Details).end();

        //append to grid
        mobileGrid.$gridContentElement.append($template);
    };

    //method will use open + overdue workorder template and will create new data row to append to grid
    mobileGrid.createOpenOverdueWorkorderRow = function (workorder) {

        //open + overdue workorder template object
        var $template = $($('#wo-mobile-grid-open-overdue-template').html());
        ...
        $template.find('.wo-jquery-mobile-grid-area').text(workorder.DepartmentAreaName).end()
                ...
                .find('.wo-jquery-mobile-grid-days-open').text(workorder.DaysOpen).end()
                .find('.wo-jquery-mobile-grid-details').text(workorder.Details).end()
                .find('.wo-jquery-mobile-grid-days-overdue').text(workorder.DaysOverdue).end();

        //append to grid
        mobileGrid.$gridContentElement.append($template);
    };

    //method will use closed workorder template and will create new data row to append to grid
    mobileGrid.createClosedWorkorderRow = function (workorder) {

        //closed workorder template object
        var $template = $($('#wo-mobile-grid-closed-template').html());
        ...
        $template.find('.wo-jquery-mobile-grid-area').text(workorder.DepartmentAreaName).end()
                 ...
                 .find('.wo-jquery-mobile-grid-days-open').text(workorder.DaysOpen).end()
                 .find('.wo-jquery-mobile-grid-details').text(workorder.Details).end()
                 .find('.wo-jquery-mobile-grid-closed-date').text(mobileGrid.formatDate(workorder.Closed)).end()
                 .find('.wo-jquery-mobile-grid-closer').text(workorder.Closer).end()
                 .find('.wo-jquery-mobile-grid-resolution').text('$' + workorder.Estimate.toFixed(2) + ' ' + workorder.Resolution).end();

        //append to grid
        mobileGrid.$gridContentElement.append($template);
    };

    //method will use closed + overdue workorder template and will create new data row to append to grid
    mobileGrid.createClosedOverdueWorkorderRow = function (workorder) {

        //closed + overdue workorder template object
        var $template = $($('#wo-mobile-grid-closed-overdue-template').html());
    ...
    $('.wo-mobile-grid-apply-filter').click(function (ev) {
        ...
        //reset filters
        mobileGrid.filter = '';
        mobileGrid.workOrderType = '';
        mobileGrid.workOrderDueType = '';
        ...
        if ($('.wo-jquery-mobile-grid-toggle-filter:first').parents('.make-switch').bootstrapSwitch('status')) {
            ...
            $('#filterModal').find('select').each(function () {
                ...
                if ($control.val() != '' && $control.attr('name') != undefined && $control.attr('name') != 'workOrderType' && $control.attr('name') != 'workOrderDueType') {

                    //create filter string
                    if (mobileGrid.filter == '') {
                        mobileGrid.filter = $control.attr('name') + '~eq~' + "'" + $control.find('option:selected').text() + "'";
                    } else {
                        mobileGrid.filter += '~and~' + $control.attr('name') + '~eq~' + "'" + $control.find('option:selected').text() + "'";
                    }

                } else if ($control.val() != '' && $control.attr('name') != undefined && $control.attr('name') == 'workOrderType') { //set workOrderType filter
                    mobileGrid.workOrderType = $control.find('option:selected').text();
                } else if ($control.val() != '' && $control.attr('name') != undefined && $control.attr('name') == 'workOrderDueType') {//set workOrderDueType filter
                    mobileGrid.workOrderDueType = $control.find('option:selected').text();
                }
    ...
    $('.wo-mobile-grid-search').click(function () {
        ...
        $('#searchModal').find('input').each(function () {
            ...
            if (controlValue != '') { //if has value
                ...
                if ($control.hasClass('wo-mobile-grid-detail-field-search')) { //if Details search field
                    mobileGrid.searchString = 'Details~contains~' + "'" + controlValue + "'"; //set Details search string

                } else { //if Resolution search field

                    //set Resolution search string
                    if (mobileGrid.searchString == '') {
                        mobileGrid.searchString = 'Resolution~contains~' + "'" + controlValue + "'";

                    } else {
                        mobileGrid.searchString += '~and~Resolution~contains~' + "'" + controlValue + "'";
                    }
                }
            }
        });
        ...
        $('#searchModal').find('input').each(function () {
            ...
            if (controlValue != '') { //if has value
                ...
                if ($control.hasClass('wo-mobile-grid-detail-field-search')) { //if Details search field
                    mobileGrid.userSearchString = '[Detail](contains)' + controlValue; //set Details search string

                } else { //if Resolution search field

                    //set Resolution search string
                    if (mobileGrid.userSearchString == '') {
                        mobileGrid.userSearchString = '[Resolution](contains)' + controlValue;
    ...
    //click event to open priority legend modal
    $('body').delegate('.wo-mobile-grid-content-row-wrapper', 'click', function (ev) {

        if (($(ev.target).length > 0 && ($(ev.target).parents('.wo-mobile-grid-content-row').length > 0 || $(ev.target).hasClass('wo-mobile-grid-content-row'))) == false) {
            $('#priorityModal').modal('show');
        }
    });

    //set default filter Status = Open
    $('#filterModal').find('select[name="workOrderType"]').val('Open');
    $('.wo-mobile-grid-apply-filter').trigger('click');
